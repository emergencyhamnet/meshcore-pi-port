...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Send one self advert through the live Pi donor runtime via a temporary localhost TCP endpoint."
    )
    ...
    parser.add_argument(
        ...
        default="flood",
        help="Advert send mode exposed by donor CMD_SEND_SELF_ADVERT",
    )
    parser.add_argument(
        "--storage-root",
    ...
    parser.add_argument("--app-name", default="pi-self-advert")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Seconds to keep the temporary live runtime running after the advert reply so RF work can drain",
    )
...
def send_self_advert(sock: socket.socket, mode: str) -> None:
    flood_flag = 1 if mode == "flood" else 0
    payload = bytes((CMD_SEND_SELF_ADVERT, flood_flag))
    send_frame(sock, payload)
    response = recv_frame(sock, "CMD_SEND_SELF_ADVERT reply")
    if len(response) != 1 or response[0] != RESP_CODE_OK:
        raise RuntimeError(f"unexpected CMD_SEND_SELF_ADVERT reply: len={len(response)} code={response[:1].hex()}")
...
def main() -> int:
    ...
    with exclusive_helper_lock():
        try:
            ...
            send_self_advert(client_socket, args.mode)
            time.sleep(args.post_send_delay)

            print("Live self advert send passed.")
