...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Set the live Pi donor runtime device clock via the companion protocol over a temporary localhost TCP endpoint."
    )
    ...
    parser.add_argument(
        "--epoch-seconds",
        type=int,
        help="UTC epoch seconds to write into the node RTC. Defaults to current host time.",
    )
    parser.add_argument(
        "--storage-root",
    ...
    parser.add_argument("--app-name", default="pi-set-device-time")
    parser.add_argument(
...
def get_device_time(sock: socket.socket) -> int:
    send_frame(sock, bytes((CMD_GET_DEVICE_TIME,)))
    response = recv_frame(sock, "CMD_GET_DEVICE_TIME reply")
    if len(response) != 5 or response[0] != RESP_CODE_CURR_TIME:
        raise RuntimeError(f"unexpected CMD_GET_DEVICE_TIME reply: len={len(response)} code={response[:1].hex()}")
    return struct.unpack("<I", response[1:5])[0]


def set_device_time(sock: socket.socket, epoch_seconds: int) -> None:
    payload = bytes((CMD_SET_DEVICE_TIME,)) + struct.pack("<I", epoch_seconds)
    send_frame(sock, payload)
    response = recv_frame(sock, "CMD_SET_DEVICE_TIME reply")
    if len(response) != 1 or response[0] != RESP_CODE_OK:
        raise RuntimeError(f"unexpected CMD_SET_DEVICE_TIME reply: len={len(response)} code={response[:1].hex()}")
...
def main() -> int:
    args = parse_args()
    epoch_seconds = args.epoch_seconds if args.epoch_seconds is not None else int(time.time())
    ...
    with exclusive_helper_lock():
        try:
            ...
            device_before = get_device_time(client_socket)
            set_device_time(client_socket, epoch_seconds)
            device_after = get_device_time(client_socket)

            print("Live device time sync passed.")
            print(f"Temporary endpoint: {endpoint}")
            print(f"Epoch seconds: {epoch_seconds}")
            print(f"UTC: {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime(epoch_seconds))}")
            print(f"Device before: {device_before}")
            print(f"Device after: {device_after}")
            print(f"Skew after sync: {device_after - epoch_seconds}")
            return 0
